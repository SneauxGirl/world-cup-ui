...
- **Visual/layout tweaks without a number:** When a visual/layout tweak doesn’t have an explicit value in the task, ask one clarifying question (measurement, screenshot reference, or “double from what?”) instead of inferring from the codebase. Prefer the smallest change that matches what I said.
- **Reference images / screenshots:** Use them for **layout and information architecture only** (section order, which controls exist, copy hierarchy) unless the user explicitly asks to match the reference’s visual styling. Always implement surfaces with this project’s WCUI patterns (`wcui-frame` chamfers, design tokens, existing component/module styles). Do not import foreign UI chrome from reference art (e.g. rounded cards, generic grey panels, pill toggles).
